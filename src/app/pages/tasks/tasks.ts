...
export class Tasks implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.timezoneSubscription = this.timezoneService.timezone$.subscribe(() => {
      console.log('Timezone changed, refreshing Task UI');
  ...
  private normalizeTask(task: any): any {
    ...
    const createdAt = task?.createdAt ?? task?.created_at ?? task?.Createdat ?? task?.createdat ?? task?.CreatedAt ?? '';
  ...
  openTaskForm(task?: any) {
    ...
    const time = task?.createdAt ? this.timezoneService.formatDateTime(task.createdAt) : '';

    // console.log("task.created at : ", task?.createdAt, "-time-", time);
    modal.innerHTML = `
        <div class="max-w-2xl mx-auto">X
          ...
          <div class="text-xs text-gray-500 mb-4 bg-white/5 p-2 rounded flex flex-wrap gap-x-4 gap-y-1">
            <span><strong>Task ID:</strong> ${task?.taskId || 'New'}</span>
            <span><strong>Start Date:</strong> ${task?.startDate || 'Not set'}</span>
            <span><strong>Due Date:</strong> ${task?.dueDate || 'Not set'}</span>
            ${time ? `<span><strong>Created At:</strong> ${time}</span>` : ''}
          </div>
  ...
  // Format due date according to user's timezone
